...
#[derive(Subcommand)] pub enum NodeAction {
    Add { name: String, #[arg(long, default_value = "paqet")] r#type: String, #[arg(long)] server: Option<String>, #[arg(long)] key: Option<String>, #[arg(long)] socks_port: Option<u16>, #[arg(long)] external_port: Option<u16> },
    List, Test { name: String }, Remove { name: String },
}
#[derive(Subcommand)] pub enum ProtoAction {
    Add { r#type: String, #[arg(long)] exit: String, #[arg(long, default_value = "auto")] port: String, #[arg(long)] force: bool }, List,
}
#[derive(Subcommand)] pub enum ImportAction {
    Add { config_link: String, #[arg(long)] name: Option<String>, #[arg(long, default_value = "auto")] port: String, #[arg(long)] exit: Option<String> },
    List, Remove { name: String }, Test { name: String },
}
#[derive(Subcommand)] pub enum PlanAction {
    Create { name: String, #[arg(long, default_value = "50GB")] data: String, #[arg(long, default_value = "30d")] duration: String, #[arg(long, default_value = "2")] devices: u32 },
    List, Remove { name: String },
}
#[derive(Subcommand)] pub enum UserAction {
    Add { username: String, #[arg(long)] plan: String },
    List, Show { username: String }, Disable { username: String }, Enable { username: String },
    Reset { username: String, #[arg(long)] extend_days: Option<u32>, #[arg(long)] reset_data: bool },
}
